...
app.get('/spin/:target', (req, res) => {
    ...
    let speedFactor = 20 //Decrease for less frames; lower load times; faster speed.
    ...
    encoder.setDelay(30); //doesn't affect encoding speed. make as large as possible without chopiness.
    ...
    let minSpeed = 0.003; //Only applied if smartScaling is enabled. Otherwise 0.002 is used.
    let maxSpeed = 0.35;
    let smartScalingReductionFactor = 3; //lower is faster, but sometimes higher is needed to seem natural.
    let degreesRequiredForSmartScaling = 270;
    ...
    let neededAngle = (angleBetweenSubdivisions * numOrder.indexOf(target)) + angleBetweenSubdivisions;
     //Extra 2 spins to make it seem more realistic
    neededAngle += (Math.PI * 2) * 2
    ...
    loadImage('./discrash.png').then((image) => {
        ...
        while (currentAngle < (neededAngle)) {
            ...
            if (smartScaling) {
                ...
                if (inSmartScalingZone) {
                    ...
                    let angleOff = ((neededAngle - currentAngle))
                    speed = (angleOff/speedFactor) + minSpeed
                    if (speed > maxSpeed) {
                        speed = maxSpeed
                    }
                }
            ...
            ctx.font = "20px Arial";
            ctx.fillStyle = "black";
            ctx.fillText(req.params.target, w/2, h/2 - (65 * ratio));
        ...
        var z = buffer.slice(offset + 19); // Remove 19 bytes responsible for looping
        let buf = [y, z]
